//
//  VrInteractiveTrackingForDisabled.m
//  Tests
//
...
@interface VrInteractiveTrackingForDisabled : XCTestCase

@end

@implementation VrInteractiveTrackingForDisabled
...
/**
 目的：インスタンスフラグがNOの状態で、初期化したインスタンスがエラーなく実行されているかを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態で、初期化したインスタンスのインスタンスフラグがNOの場合でもインスタンスが生成できているかを確認
 */
...
/**
 目的：インスタンスフラグがNOの状態で、初期化時の引数にnullを入れた場合でも、インスタンが生成されることを確認する
 */
- (void)testInitWithTracker3IrregularNull {
    VrInteractiveData *data = [VrInteractiveData sharedInstance];
    VrInteractiveTracking *tracking = [data withClass:self withAppName:nil withEventName:nil withMonitorId:nil withOptFlg:NO withOutsideConfigURL:nil];
...
/**
 目的：インスタンスフラグがNOの状態で、初期化時の引数に空文字を入れた場合でも、インスタンが生成されることを確認する
 */
- (void)testInitWithTracker3IrregularEmpty {
    VrInteractiveData *data = [VrInteractiveData sharedInstance];
    VrInteractiveTracking *tracking = [data withClass:self withAppName:@"" withEventName:@"" withMonitorId:@"" withOptFlg:NO withOutsideConfigURL:@""];
...
/**
 目的：インスタンスフラグがNOの状態で、２つのインスタンが別のインスタンスであることを確認する
 */
- (void)testInitWithTracker3IrregularEquivalence {
    VrInteractiveData *data = [VrInteractiveData sharedInstance];
    VrInteractiveTracking *tracking1 = [data withClass:self withAppName:@"test_app_name1" withEventName:@"test_event_name1" withMonitorId:@"test_monitor_id1" withOptFlg:NO withOutsideConfigURL:nil];
    VrInteractiveTracking *tracking2 = [data withClass:self withAppName:@"test_app_name2" withEventName:@"test_event_name2" withMonitorId:@"test_monitor_id2" withOptFlg:NO withOutsideConfigURL:nil];
...
/**
 目的：インスタンスフラグがNOの状態で、不正な設定ファイルのURLを設定した場合でも、インスタンスが生成できているかを確認する
 */
- (void)testInitWithTracker3IrregularWrongURL {
    VrInteractiveData *data = [VrInteractiveData sharedInstance];
    VrInteractiveTracking *tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" withOptFlg:NO withOutsideConfigURL:@"ht://config.jp/"];
...
/**
 目的：インスタンスフラグがNOの状態で、初期化したインスタンスがインスタンスフラグがNOの場合でも生成できているかを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態で、不正なURLを引数に入れた場合、初期化コールバックでNOが返ってくることを確認する。
 */
- (void)testInitWithTracker4IrregularWrongURL {
    ...
    tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" finishInitBlock:^(BOOL result) {
        ...
        // 前提：ユーザーがオプトアウトしている。不正なURLを使用して初期化が終了している
        // 想定：不正なURLはエラーを吐き出した上で、nilに丸められるので、初期化コールバック結果がfalseであることを確認（別途エラーを吐き出している）
        XCTAssertFalse(result);
        // 想定：コールバックが後に返されるので、初期化処理が終わっているのではnilになっていない事を確認
        XCTAssertNotNil(tracking);
        
        [expectation fulfill];
    } withOptFlg:NO withOutsideConfigURL:@"ht://config.jp/"];
...
/**
 目的：インスタンスフラグがNOの状態で、UUIDを取得した際に、nilではあることを確認する
 */
- (void)testUuIdNormal {
    __weak XCTestExpectation *expectation = [self expectationWithDescription:@"CallApiBlocks"];
    VrInteractiveData *data = [VrInteractiveData sharedInstance];
...
/**
 目的：インスタンスフラグがNOの状態で、UUIDの更新をした場合でsもエラーを吐かないことを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態でUUIDを更新した場合でもUUIDが空文字でもエラーを吐かないことを確認する
 */
...
//
/**
 目的：インスタンスフラグがNOの状態で、UUIDにnullを上書きした際にエラーを吐かないことを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態で、UUIDのUpdateを行ってもエラーを吐かないことを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態で、Opt値をクリアしてもエラーを吐かないかを確認する
 */
...
/**
 目的：インスタンスフラグがNOの状態で、opt値のクリアを複数回呼び出してもエラーが発生しないかを確認する
 */
...
- (void)testLoadConfigCallbackIrregularNull {
    ...
    tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" finishInitBlock:^(BOOL result) {
        ...
        [tracking loadConfig:nil fileName:nil finishBlock:^(BOOL result) {
            ...
            // 想定：identityがdefaultとしてコールバックが登録されているので、falseで返ってくることを確認
            XCTAssertTrue(result == NO);
...
- (void)testConfigParamsNormal {
    ...
    tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" finishInitBlock:^(BOOL result) {
        ...
        // 想定：設定値がnilという事を確認
        XCTAssertNil([tracking configParams]);
...
- (void)testConfigParamsWithIdentityNormal {
    ...
    tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" finishInitBlock:^(BOOL result) {
        ...
        [tracking loadConfig:@"test" fileName:@"TestVrTrackingConfig" finishBlock:^(BOOL result) {
            ...
            // 前提：ユーザーがオプトアウトしている。"test"というidentityで設定ファイルを読みが完了している
            // 想定：設定値がnilという事を確認
            XCTAssertNil([tracking configParams:@"test"]);
...
/**
 目的：nullのidentityを指定しても読み込んだ設定ファイルの取得を行うとnilが返ってくる事を確認する
 */
- (void)testConfigParamsWithIdentityIrregularNull {
    ...
    tracking = [data withClass:self withAppName:@"test_app_name" withEventName:@"test_event_name" withMonitorId:@"test_monitor_id" finishInitBlock:^(BOOL result) {
        ...
        [tracking loadConfig:@"test" fileName:@"TestVrTrackingConfig" finishBlock:^(BOOL result) {
            ...
            // 前提：ユーザーがオプトアウトしている。"test"というidentityで設定ファイルを読みが完了している
            // 想定：設定値がnilという事を確認
            XCTAssertNil([tracking configParams:nil]);
...
/**
 目的：空文字のidentityを指定しても読み込んだ設定ファイルの取得を行うとnilが返ってくる事を確認する
 */
